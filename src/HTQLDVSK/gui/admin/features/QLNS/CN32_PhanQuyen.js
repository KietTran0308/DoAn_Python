(() => {
    ...
    async function loadBaseDataFromAPI() {
        try {
            // Đổi URL fetch chức năng sang API riêng để lấy thứ tự Cha - Con chuẩn xác
            const [rolesRes, funcsRes] = await Promise.all([
                fetch('http://localhost:8000/api/nhom-quyen'),
                fetch('http://localhost:8000/api/phan-quyen/chuc-nang')
            ]);
            ...
            window.Toast.warning("Lỗi kết nối Server.");
        }
    }
    ...
    function setupEvents() {
        ...
        if (!window.hasPermission(32, 'SUA')) {
    ...
    function renderPermissionTable() {
        ...
        functionsData.forEach(fn => {
            ...
            actions.forEach(action => {
                ...
                // ==========================================
                // CHỈ CHỈNH SỬA Ở ĐÂY: XÓA Ô TICK CỦA CHỨC NĂNG CHA
                // ==========================================
                if (fn.MA_CN_CHA === null && action !== 'XEM') {
                    td.innerHTML = '<span style="color: #ccc;">-</span>';
                    tr.appendChild(td);
                    return; // Bỏ qua hoàn toàn việc tạo checkbox bên dưới
                }
        ...
        // Ẩn thanh pagination đi vì không còn sử dụng
