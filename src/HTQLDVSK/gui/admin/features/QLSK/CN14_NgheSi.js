(() => {
    ...
    function init() {
        if (!window.hasPermission(14, 'THEM')) {
    ...
    function openArtistModal(artist) {
        ...
        if (artist) {
            // Chế độ Xem & Cập nhật
            title.textContent = "Chi tiết Nghệ Sĩ"; // Đổi tên cho hợp ngữ cảnh
            ...
            // Nếu không có quyền XÓA thì ẩn nút Xóa
            deleteBtn.style.display = window.hasPermission(14, 'XOA') ? "block" : "none";

            // Nếu không có quyền SỬA thì khóa Form lại và ẩn nút Lưu
            const btnSave = document.getElementById('save-artist-btn');
            if (!window.hasPermission(14, 'SUA')) {
                document.getElementById('TEN_NS').disabled = true;
                document.getElementById('IMAGE_URL').disabled = true;
                document.getElementById('TIEU_SU').disabled = true;
                if(btnSave) btnSave.style.display = 'none';
            } else {
                title.textContent = "Chi tiết / Cập nhật Nghệ Sĩ";
                document.getElementById('TEN_NS').disabled = false;
                document.getElementById('IMAGE_URL').disabled = false;
                document.getElementById('TIEU_SU').disabled = false;
