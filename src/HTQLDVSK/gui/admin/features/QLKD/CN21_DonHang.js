(() => {
    ...
    function renderOrderTable() {
        ...
        pageData.forEach(order => {
            ...
            tr.innerHTML = `
                <td><b>${order.MA_DH.toString().padStart(5, '0')}</b></td>
    ...
    function openOrderModal(order) {
        ...
        if (order.TRANG_THAI === 1) {
            ...
            if(updateBtn) {
                ...
                updateBtn.onclick = () => {
                    window.Toast.success(`Đã xác nhận thanh toán thủ công cho ĐH${order.MA_DH.toString().padStart(5, '0')}!`);
                    order.TRANG_THAI = 1;
    ...
    function exportToExcel() {
        ...
        if (currentOrderData.length === 0) {
            window.Toast.warning("Không có dữ liệu đơn hàng nào để xuất!");
            return;
        }
